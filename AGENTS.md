...
Itrium's Homebrew tap. Each file in `Casks/` installs one of Itrium's applications from its
...
- **Before committing, lint the cask.** Homebrew only lints casks inside a tap, so link this
  checkout in as the tap (`brew untap itriumid/tap` first if it's tapped from GitHub):
  `ln -sfn "$PWD" "$(brew --repository)/Library/Taps/itriumid/homebrew-tap"`.
  Then run `brew style --cask itriumid/tap/<cask>` and
  `brew audit --cask --strict --online itriumid/tap/<cask>`, and fix everything they report.
- **Our applications are unsigned,** so every cask carries a `caveats` block explaining Open
  Anyway and the `xattr` command, and links to the application's README section on it. Remove
